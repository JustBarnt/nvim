      ...
      mr.refresh(function()
        for _, tool in ipairs(opts.ensure_installed) do
          ...
          if not p:is_installed() then
            ...
            p:install():once("closed",
              vim.schedule_wrap(function()
                if p:is_installed() then
                  vim.notify(("[mason.nvim] %s was installed successfully"):format(p.name))
                else
                  vim.notify(("[mason.nvim] failed to install %s. Installation logs are available in :Mason and :MasonLog"):format(p.name), vim.log.levels.ERROR)
                end
              end)
            )
          end
